...
class MongoDBUtils(object):
    ...
    def __init__(self):
        ...
        self.mongo_client = pymongo.MongoClient(
            host=MONGO_DB_HOST, port=MONGO_DB_PORT)
        self.mongo_client[MONGO_DB_NAME].authenticate(
            "root", "password", mechanism='SCRAM-SHA-1')
    ...
    def save_tweet(self, document, source):
        try:
           ...
           # document["follow"] = True if EnumSource.FOLLOW in source else False
            ...
            self.logger.error(
                'Error while trying to save user account', exc_info=True)
    ...
    def set_profilePic_age_gender_user(self, screen_name, age, gender):
        ...
        col.update({'screen_name': screen_name}, {
                   '$set': {'profile_pic_age': age}})
        col.update({'screen_name': screen_name}, {
                   '$set': {'profile_pic_gender': gender}})

    def updateProfilePicture(self, screen_name, imageUrl):
        db = self.mongo_client[MONGO_DB_NAME]
        col = db[DB_COL_USERS]
        col.update({'screen_name': screen_name}, {
                   '$set': {'profile_image_url_https': imageUrl}})

    def save_listSubscriptions(self, screen_name, lists):
        db = self.mongo_client[MONGO_DB_NAME]
        col = db[DB_COL_USERS]
        col.update({'screen_name': screen_name}, {
                   '$set': {'listsSubscriptions': lists}})
    ...
    def get_customFields(self, typeOp):
        ...
        try:
            df = DataFrame(columns=('screen_name', 'friends_count',  'tweets_count', 'linkedin', 'snapchat', 'instagram', 'facebook',
                                    'followers_count', 'favourites_count', 'qtyMentions', 'qtyHashtags', 'qtyUrls', 'qtyEmojis', 'profile_pic_gender', 'age'))
            # Obtiene una referencia a la instancia de la DB
            db = self.mongo_client[MONGO_DB_NAME]
            # Obtiene el ObjectID Mongo del perfil del data source para el usuario
            col = db[DB_COL_USERS]
            ...
            for user in col.find():  # para cada usuario
                ...
                df.loc[count] = [user['screen_name'], user['friends_count'], user['statuses_count'], user['linkedin'], user['snapchat'], user['instagram'], user['facebook'],
                                 user['followers_count'], user['favourites_count'], user['qtyMentions'], user['qtyHashtags'], user['qtyUrls'], user['qtyEmojis'], gender, age]
                count += 1
            return df
    ...
    def get_profilePicAgeDataset(self):
        ...
        try:
            ...
            count = 0
            for user in col.find():  # para cada usuario
                df.loc[count] = [user['screen_name'],
                                 user['profile_pic_age'], user['age']]
                count += 1
            return df
    ...
    def getBioWithAge(self, collection):
        ...
        for user in col.find({}):  # para cada unlabeled user
            print "etiquetando user..."
        
            bio = user['description']
            print user['screen_name']
            year = int(user['created_at'][-4:])
            ...
            for user in col.find():  # para cada usuario
                ...
                if user['qtyMentions'] == 0 and user['qtyHashtags'] == 0 and user['qtyUrls'] == 0 and user['qtyEmojis'] == 0:
                    ...
                    if round(qtyMentions/qtyTweets, 2) != user['qtyMentions']:
                        print "cambio qtyMentions ",  round(
                            qtyMentions/qtyTweets, 2), '"-', user['qtyMentions']
                        col.update({'screen_name': user['screen_name']}, {
                                   '$set': {'qtyMentions': round(qtyMentions/qtyTweets, 2)}})

                    if round(qtyHashtags/qtyTweets, 2) != user['qtyHashtags']:
                        print "cambio qtyHashtags ",  round(
                            qtyHashtags/qtyTweets, 2), '"-', user['qtyHashtags']
                        col.update({'screen_name': user['screen_name']}, {
                                   '$set': {'qtyHashtags': round(qtyHashtags/qtyTweets, 2)}})

                    if round(qtyUrls/qtyTweets, 2) != user['qtyUrls']:
                        print "cambio qtyUrls ",  round(
                            qtyUrls/qtyTweets, 2), '"-', user['qtyUrls']
                        col.update({'screen_name': user['screen_name']}, {
                                   '$set': {'qtyUrls': round(qtyUrls/qtyTweets, 2)}})

                    if round(qtyEmojis/qtyTweets, 2) != user['qtyEmojis']:
                        print "cambio qtyEmojis ",  round(
                            qtyEmojis/qtyTweets, 2), '"-', user['qtyEmojis']
                        col.update({'screen_name': user['screen_name']}, {
                                   '$set': {'qtyEmojis': round(qtyEmojis/qtyTweets, 2)}})

                    if round(qtyUppercase/qtyTweets, 2) != user['qtyUppercase']:
                        print "cambio qtyUppercase ",  round(
                            qtyUppercase/qtyTweets, 2), '"-', user['qtyUppercase']
                        col.update({'screen_name': user['screen_name']}, {
                                   '$set': {'qtyUppercase': round(qtyUppercase/qtyTweets, 2)}})
    ...
    def export_tweetsText_toCSV(self, typeOp, faceApi):
        ...
        try:
            df = DataFrame(columns=('screen_name', 'friends_count',  'tweets_count', 'linkedin', 'snapchat', 'instagram', 'facebook', 'followers_count',
                                    'favourites_count', 'qtyMentions', 'qtyHashtags', 'qtyUrls', 'qtyEmojis', 'qtyUppercase', 'profile_pic_gender', 'tweets', 'age','profile_pic_age'))
            # Obtiene una referencia a la instancia de la DB
            db = self.mongo_client[MONGO_DB_NAME]
            # Obtiene el ObjectID Mongo del perfil del data source para el usuario
            col = db[DB_COL_USERS]
            ...
            for user in cursor:  # para cada usuario
                ...
                df.loc[count] = [user['screen_name'], user['friends_count'], user['statuses_count'], linkedin, snapchat, instagram, facebook, user['followers_count'],
                                 user['favourites_count'], user['qtyMentions'], user['qtyHashtags'], user['qtyUrls'], user['qtyEmojis'], user['qtyUppercase'], gender, tweetText, age,user['profile_pic_age']]
                count += 1
            ...
            if faceApi == 'faceAPI':
                ...
                train_data.to_csv(
                    typeOp+"_faceAPI_tweets_train.csv", index=False)
                test_data.to_csv(
                    typeOp+"_faceAPI_tweets_test.csv", index=False)
            else:
    ...
    def export_tweetsText_toCSV_balanced(self):
        ...
        try:
            df = DataFrame(columns=('screen_name', 'friends_count',  'tweets_count', 'linkedin', 'snapchat', 'instagram', 'facebook', 'followers_count',
                                    'favourites_count', 'qtyMentions', 'qtyHashtags', 'qtyUrls', 'qtyEmojis', 'qtyUppercase', 'profile_pic_gender', 'tweets', 'age'))
            # Obtiene una referencia a la instancia de la DB
            db = self.mongo_client[MONGO_DB_NAME]
            # Obtiene el ObjectID Mongo del perfil del data source para el usuario
            col = db[DB_COL_USERS]
    ...
    def export_subscriptionLists_toCSV(self, typeOp, faceApi):
        ...
        try:
            ...
            if faceApi == 'faceAPI':
                cursor = col.find(
                    {"profile_pic_age": {"$ne": -1}, "listsSubscriptions": {"$nin": [[], -1]}})
            else:
            ...
            if faceApi == 'faceAPI':
                df.to_csv(
                    typeOp+"_faceAPI_subscriptionLists_COMPLETE.csv", index=False)
                train_data.to_csv(
                    typeOp+"_faceAPI_subscriptionLists_train.csv", index=False)
                test_data.to_csv(
                    typeOp+"_faceAPI_subscriptionLists_test.csv", index=False)
            else:
                df.to_csv(typeOp+"_subscriptionLists_COMPLETE.csv", index=False)
                train_data.to_csv(
                    typeOp+"_subscriptionLists_train.csv", index=False)
                test_data.to_csv(
                    typeOp+"_subscriptionLists_test.csv", index=False)
    ...
    def export_subscriptionLists_toCSV_balanced(self):
        ...
        try:
                ...
                for user in col.find({"listsSubscriptions": {"$nin": [[], -1]}, "age": ageR}).limit(11):
                    ...
                    try:
                        if user['listsSubscriptions'] != -1 and len(user['listsSubscriptions']) > 0:
                            ...
                            df.loc[count] = [
                                user['screen_name'], subsLists, age]
                            count += 1
                        else:
            ...
            train_data.to_csv(
                "subscriptionLists_balanced_train.csv", index=False)
            test_data.to_csv(
                "subscriptionLists_balanced_test.csv", index=False)
    ...
    def export_customFields(self, typeOp, faceApi):
        ...
        try:
            df = DataFrame(columns=('screen_name', 'friends_count',  'tweets_count', 'linkedin', 'snapchat', 'instagram', 'facebook', 'followers_count',
                                    'favourites_count', 'qtyMentions', 'qtyHashtags', 'qtyUrls', 'qtyEmojis', 'qtyUppercase', 'profile_pic_gender', 'age'))
            # Obtiene una referencia a la instancia de la DB
            db = self.mongo_client[MONGO_DB_NAME]
            # Obtiene el ObjectID Mongo del perfil del data source para el usuario
            col = db[DB_COL_USERS]
            ...
            for user in cursor:  # para cada usuario
                ...
                df.loc[count] = [user['screen_name'], user['friends_count'], user['statuses_count'], linkedin, snapchat, instagram, facebook, user['followers_count'],
                                 user['favourites_count'], user['qtyMentions'], user['qtyHashtags'], user['qtyUrls'], user['qtyEmojis'], user['qtyUppercase'], gender, age]
                count += 1
            ...
            if faceApi == 'faceAPI':
                df.to_csv(
                    typeOp+"_faceAPI_customFields_COMPLETE.csv", index=False)
                train_data.to_csv(
                    typeOp+"_faceAPI_customFields_train.csv", index=False)
                test_data.to_csv(
                    typeOp+"_faceAPI_customFields_test.csv", index=False)
            else:
                df.to_csv(typeOp+"_customFields_COMPLETE.csv", index=False)
                train_data.to_csv(
                    typeOp+"_customFields_train.csv", index=False)
